...
class CarouselParameters {
    ...
    companion object {
        ...
        fun <T : CarouselView1.ViewTransformer> getDefaultTransformerParameters(
            ...
            for (setterMethod in setterMethods) {
                ...
                try {
                    ...
                    Log.w("CarouselViewDemo", "Cannot find getter method " + getterMethodName + " from " + clazz.name)
                    e.printStackTrace()
                } catch (e: InvocationTargetException) {
                    throw RuntimeException(e)
                } catch (e: IllegalAccessException) {
        ...
        private fun <A : Annotation> findAnnotation(annotations: Array<Annotation>?, type: Class<A>): A? {
        ...
        fun getParameterRangeInfoDouble(setterMethod: Method): RangeInfo<Double> {
            ...
            val annotatedRange = findAnnotation(setterMethod.parameterAnnotations[0], FloatRange::class.java)
        ...
        fun getParameterRangeInfoInt(setterMethod: Method): RangeInfo<Int> {
            ...
            val annotatedRange = findAnnotation(setterMethod.parameterAnnotations[0], IntRange::class.java)
            if (annotatedRange != null) {
                val from =
                    annotatedRange.from // + (annotatedRange.fromInclusive() ? 0 : FLOAT_PARAMETER_EXCLUSIVE_OFFSET);
                val to = annotatedRange.to // - (annotatedRange.toInclusive() ? 0 : FLOAT_PARAMETER_EXCLUSIVE_OFFSET);
                lower = if (from > Integer.MIN_VALUE) from.toInt() else INT_PARAMETER_DEFAULT_MIN_VALUE
